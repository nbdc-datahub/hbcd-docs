...
<div style="flex: 1;">
<p>The <b>StimTracker</b> device, used during EEG acquisition for visits V03, V04, and V06, provides precise timing for auditory and visual stimulus onset and marks task start and end points. The EEG Core identified an <b>electrical artifact</b> at stimulus onset and offset, caused by intermittent spikes from the device. This artifact appears in a subset of files collected with current HBCD systems.</p>
<p>To detect the artifact, the EEG Core developed a custom MATLAB script that averages all epochs per task and flags voltage fluctuations >1 µV in two time windows: –10 to 10 ms (onset) and 250 to 270 ms (offset).<p>
</div>
  <!-- Image on the right -->
...
Based on current evaluation, the **StimTracker artifact does not meaningfully affect mean ERP amplitudes** in the released derivatives. Statistical analyses comparing mean amplitudes between artifact-free and artifact-present files (after FDR correction) found only two significant differences, both in the T7T8 cluster for the MMN task (standard and deviant conditions). No significant differences were observed for difference waves (Predeviant–Deviant) for the FCz and T7T8 clusters or for the VEP and FACEs tasks.

Overall, the StimTracker artifact has a **minimal impact** on the data. However, we encourage users to assess potential effects in their own analyses using the eeg_qc_-<task> variable present in the task-level EEG_QC.csv file (generated by the Lasso tabulated query). This binary variable indicates whether the artifact was present (yes/no) in the processed data.

Due to limited data, no comparisons were conducted for V04 files.

See [Effects of Artifact on ERP Derivatives by Task](#effects-of-artifact-on-erp-derivatives-by-task) for more information about the StimTracker artifact.
