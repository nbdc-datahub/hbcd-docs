...
For structural and functional MRI processing, only files that passed [raw data quality control](qc.md#raw-mr-data-qc) are utilized for processing. This is based on thresholds for `QU_motion`, `acq_motion`, `brain_SNR`, etc. queried from the `sub-{ID}_ses-{V0X}_scans.tsv` file located in each BIDS session folder. When additional QC criteria apply, filtering typically occurs in two stages: first, using both manual and automated QC fields, and second, using only automated fields. For example, for structural MRI processing, when multiple scans passing QC are present, the single highest-quality T1w and T2w are selected for each session based on `QU_Motion`, a manual assessment of motion artifacts.
