...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const domainSelect = document.getElementById("issue-domain");
  const typeSelect = document.getElementById("issue-type");
  const releaseSelect = document.getElementById("issue-release");
  const clearButton = document.getElementById("issue-clear");
  const countDisplay = document.getElementById("issues-count");

  const rows = Array.from(
    document.querySelectorAll("#issues-table .issue-row")
  );

  const domainRows = Array.from(
    document.querySelectorAll("#issues-table .domain-row")
  );

  // --------------------------------------------------
  // Populate dropdowns from the table
  // --------------------------------------------------
  ...
  populateSelect(domainSelect, uniqueValues("domain"));
  populateSelect(typeSelect, uniqueValues("type"));
  populateSelect(releaseSelect, uniqueValues("release"));
  ...
  function filterIssues() {
    ...
    // Show/hide domain headers based on whether they
    // have any visible issue rows.
    domainRows.forEach(domainRow => {

      const domain = domainRow.dataset.domain;

      const hasVisibleIssues = rows.some(row =>
        row.dataset.domain === domain &&
        row.style.display !== "none"
      );

      domainRow.style.display =
        hasVisibleIssues ? "" : "none";
    });

    countDisplay.textContent =
      `Showing ${visibleCount} of ${rows.length} issues`;
  }
  ...
  domainSelect.addEventListener("change", filterIssues);
  typeSelect.addEventListener("change", filterIssues);
  releaseSelect.addEventListener("change", filterIssues);
