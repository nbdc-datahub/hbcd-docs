...
In some cases, a specimen meets the threshold for a positive result, but the confirmatory result is reported as “QNS” (Quantity Not Sufficient). This occurs when a confirmatory test must be repeated (following quality control procedures, e.g. when a sample result is &lt;5% of the previous specimen), but insufficient sample remains to complete the repeat test. Because the substance concentration could not be validated, this should be treated as a true quantity insufficiency.</p>
</div>
