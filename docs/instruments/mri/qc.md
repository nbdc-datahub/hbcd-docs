...
This section describes the criteria used to identify MRI derivatives that require manual review or exclusion from the data release. 

#### BrainSwipes QC Thresholds That Trigger Manual Review

Session data that don't meet the following BrainSwipes QC thresholds are flagged for additional manual review:

 - **Structural (T1w/T2w):** Flagged if the mean QC rating across BrainSwipes structural visual reports is < 0.7.
 - **Functional:** Each BOLD run is independently flagged if the mean QC rating across its visual reports is < 0.5.

<p style="color: red;">LUCI ADMIN NOTE: ADD REASONING FOR THESE THRESHOLDS</p>

#### Manual Review & Exclusion Procedures

When BrainSwipes QC identifies structural or functional failures, the corresponding derivatives undergo manual inspection to determine the earliest processing stage affected. Exclusion decisions follow the dependency structure of the pipelines (BIBSNet → Infant fMRIPrep → XCP-D).

 - **Structural QC failures**: outputs from each pipeline (BIBSNet, Infant fMRIPrep, and XCP-D) are inspected to identify which processing stage resulted in poor quality outputs. Derivatives are removed for pipeline outputs for stage X forward (e.g. if serious issues are identified in the BIBSNet outputs, then derivatives for that session are excluded from the release for BIBSNet through XCP-D, both structural and functional).
 - **Functional QC failures**: outputs from Infant fMRIPrep and XCP-D are inspected to identify the source of the issue and remove all functional outputs from either Infant fMRIPrep and XCP-D or XCP-D alone 

<p style="color: red;">LUCI ADMIN NOTE: PROBABLY NEED TO DESCRIBE IN GREATER DETAIL WHAT WE MEAN BY "SERIOUS ISSUES" IDENTIFIED FOR EACH PIPELINE AND ALSO LAY OUT THE EXCLUSION LOGIC/DECISION TREE WAY MORE CLEARLY</p>
