...
For the 1.0 and 1.1 releases, TSV and Parquet files were created from source data using separate pipelines, which caused some discrepancies. Software and library differences in converting PHP code to Parquet introduced variation in the precision of floating-point values (<code>type_data</code>=<i>doubles</i>) relative to the TSV tables as well as some additional differences with regard to data types. To address this issue and ensure consistency between the two formats, data for future releases will  be created in one step.
</td>
</tr>
</tbody>
</table>
