...
<div style="flex: 1;">
<p>Quality control of processed structural and functional MRI data is conducted via manual review of visual reports produced by XCP-D. Manual inspection remains the gold standard for this type of QC, but is incredibly resource-intensive.</p>
<p>To support large-scale QC, HBCD uses <a href="https://brainswipes.us/about/">BrainSwipes</a>, a gamified, crowdsourced platform where users complete a brief tutorial on visual QC and classify images as pass or fail by swiping right or left.</p>
<p>Note that BrainSwipes QC results were also used to identify and exclude a subset of derivative outputs with severe data quality issues from the release: see <a href="../exclusion-criteria/#processed-data-exclusion-criteria" target="_blank">Processed Data Exclusion Criteria</a> for details.</p>
</div>
  <!-- Image on the right -->
...
Each modality (T1w/T2w and BOLD runs) has a series of XCP-D visual reports that are each independently rated as **Pass (1)** or **Fail (0)** (expand  [infobox below](#swipes-procedures) below for full procedural details). The BrainSwipes release dataset includes:
