...
### Raw BIDS
Raw BIDS (`rawdata/`) raw data in BIDS standard format provided for imaging, EEG, and biosensor recordings, organized under separate subject, session, and modality-specific subfolders according to the [BIDS](https://bids-specification.readthedocs.io/en/stable/) standard. Raw BIDS also includes [dataset metadata](#participant-session-and-scan-level-metadata). Click on the links to see detailed folder contents for each modality:
...
<p>
<div id="participant-session-and-scan-level-metadata" class="banner" onclick="toggleCollapse(this)">
<span class="emoji">
<i class="fa-solid fa-diagram-project"></i>
</span>
<span class="text-with-link">
    <span class="text">Participant-, session-, and scan-level metadata</span>
    <a class="anchor-link" href="#participant-session-and-scan-level-metadata" title="Copy link">
        <i class="fa-solid fa-link"></i>
    </a>
</span>
<span class="arrow rotate">▸</span>
...
### Derivatives 
Derivatives (`derivatives/`) are generated by processing the raw BIDS through standardized, modality-specific [processing pipelines](../standards/pipelines.md). These again are provided for imaging, EEG, and biosensor recordings organized according to the BIDS standard. 
